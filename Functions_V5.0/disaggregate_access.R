...
disaggregate_access = function(aoi_border,
  ...
  if (!is.na(aoi_access_ll)) {
    if (st_geometry_type(aoi_access_ll,by_geometry=FALSE) != "POLYGON" &
      ...
      aoi_access_ll<-st_collection_extract(
        aoi_access_ll,type=c("POLYGON","POINT","LINESTRING"))
      aoi_access_ll<-st_union(aoi_access_ll) %>%
        st_sf()
    }
  }
  ...
  # Write to disk
  if (is.na(aoi_access_ll)) {
    st_write(aoi_remote_ll,paste0(
      destination,aoi_prefix,"remote.shp"),
      delete_layer=TRUE,quiet=TRUE)
  } else {
    st_write(aoi_remote_ll,paste0(
      destination,aoi_prefix,"remote.shp"),
      delete_layer=TRUE,quiet=TRUE)
    st_write(aoi_access_ll,paste0(
      destination,aoi_prefix,"accessible.shp"),
      delete_layer=TRUE,quiet=TRUE)
  }
